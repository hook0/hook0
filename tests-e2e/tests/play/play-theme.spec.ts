...
test.describe("Play Theme", () => {
  ...
  test("theme preference persists in localStorage across page reload", async ({ page }) => {
    await page.goto("/");
    await expect(page.locator("#webhookUrl")).toBeVisible({ timeout: 10000 });
    ...
    const savedTheme = await page.evaluate(() => localStorage.getItem("hook0play_theme"));
  ...
  test("default theme is dark when no preference is saved", async ({ browser, baseURL }) => {
