...
-- Audit log of subscription health transitions. Each row records the moment
-- a subscription's perceived health changed: warning triggered, auto-disabled,
-- manually re-enabled, or recovered. The subscription health monitor appends
-- rows as it detects failure-rate threshold crossings; the subscriptions API
-- appends rows for manual actions. The health timeline endpoint reads this
-- table to reconstruct a subscription's history.
...
-- Buckets are bounded by EITHER duration (subscription_health_bucket_duration) OR message
-- count (subscription_health_bucket_max_messages), whichever limit is reached first.
-- The subscription health monitor evaluates subscription health by summing recent buckets
-- within the configured failure_rate_evaluation_window.
...
-- which deletes buckets where bucket_start < now() - bucket_retention_days.
...
-- On each tick the subscription health monitor only reads deliveries newer than this
-- cursor, avoiding a full table scan of request_attempt every time.
...
-- Denormalized failure_percent cache on the subscription row. Computed by the subscription
-- health monitor from the buckets table and read directly by the subscriptions API, avoiding
-- a bucket join on every subscription list/get call.
