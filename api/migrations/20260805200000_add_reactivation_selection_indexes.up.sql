...
-- as iam.user grows. `select_candidates` joins iam.organization on `created_by`
-- and narrows verified users to a sign-up-age window, so both columns get an
-- index and the user-side one is partial: only verified accounts are ever
