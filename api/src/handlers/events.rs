...
pub struct EventPost {
    ...
    application_id: Uuid,
    /// Optional unique identifier for this event (client-generated UUID). If not provided, a UUIDv7 will be generated by the server.
    ...
    if can_ingest {
        ...
        let event = query_as!(
                ...
                "
                    ...
                    VALUES ($1, COALESCE($2, uuidv7()), $3, $4, $5, $6, $7, $8, statement_timestamp(), $9)
...
/// Timeout for acquiring the Pulsar producer Mutex lock.  3s matches the
/// existing `send_request_attempts_to_pulsar` pattern.  If the lock is held
/// longer than this, the producer is likely stuck and we should fail fast.
...
/// the message.  Best-effort: returns `Ok(())` even if no Pulsar worker is
/// configured for this subscription (the PG poller will pick it up instead).
pub(crate) async fn send_single_attempt_to_pulsar(
    db: &sqlx::PgPool,
    pulsar: &Arc<PulsarConfig>,
    ...
    if let Some(r) = route
        && let Some(worker_id) = r.worker_id
        && r.worker_queue_type.as_deref() == Some("pulsar")
    {
        let mut producer = timeout(
            PULSAR_PRODUCER_LOCK_TIMEOUT,
            pulsar.request_attempts_producer.lock(),
        ...
        if let Err(e) = producer
            ...
            .await
        {
            error!("Failed to send attempt to Pulsar: {e}");
        }
    }
