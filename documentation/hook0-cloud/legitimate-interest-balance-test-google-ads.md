...
| | |
|---|---|
| **Traitement** | Transmission du gclid à Google Ads pour mesure de trois conversions publicitaires server-side : (1) « Signup » à la vérification d'email ; (2) « Activation » à la création du premier jeton d'accès de l'organisation (clé API ou jeton de service) ; (3) « First event sent » au premier événement effectivement ingéré par l'organisation |
...
Dès que les conversions applicables sont uploadées, le gclid est mis à `NULL` dans la table (`gclid = NULL`) : la ligne subsiste jusqu'au cleanup mais ne contient plus la donnée pseudonyme. Lorsque la conversion « First event sent » est active, la nullification requiert les trois uploads (`signup_uploaded_at`, `activation_uploaded_at` et `first_event_uploaded_at` non nuls) ; lorsqu'elle est inactive, elle requiert les deux uploads Signup et Activation comme précédemment. La réponse de Google n'est jamais attendue ni bloquante : les handlers utilisateur (vérification d'email, création de jeton, ingestion d'événement) réussissent indépendamment de l'issue des uploads.
...
| Nullification du gclid dès que les uploads applicables sont réalisés (minimisation art. 5.1.e) | Réalisée | Requête `UPDATE iam.signup_attribution SET gclid = NULL WHERE … AND signup_uploaded_at IS NOT NULL AND activation_uploaded_at IS NOT NULL` — étendue à `AND first_event_uploaded_at IS NOT NULL` lorsque la conversion « First event sent » est active — exécutée après chaque upload. La ligne subsiste pour le cleanup mais ne contient plus de donnée pseudonyme. |
...
| Opposition art. 21.2 traitée manuellement (pas de drapeau d'opt-out persistant par utilisateur) : une ligne d'attribution pourrait être uploadée avant le traitement manuel de la demande | Faible | Rétention bornée (gclid nullifié après upload des conversions applicables, ligne purgée sous 90 j au plus tard) ; suppression manuelle de la ligne `iam.signup_attribution` sur demande ; demande de suppression à Google pour les données déjà transmises. Drapeau d'opposition persistant par utilisateur à implémenter si le volume d'oppositions le justifie. |
...
| Rétention du gclid prolongée par l'attente des conversions ultérieures (Activation, puis First event sent lorsqu'elle est active) | Très faible | La nullification intervient dès que les uploads applicables sont réalisés ; la durée effective est généralement de quelques heures à quelques jours ; la borne absolue est de 30 jours, alignée sur la fenêtre d'attribution au clic des actions de conversion, au-delà de laquelle Google refuse l'upload (cf. § 3.3) |
...
flowchart LR
    ...
    FirstEvJob -->|14. scan orgs gclid + événement<br/>si conversion active| DB
    FirstEvJob -.->|15. upload — First event conversion| GAdsAPI
    FirstEvJob -->|16. UPDATE first_event_uploaded_at| DB
    DB -->|17. uploads applicables ok ?<br/>SET gclid = NULL| Null
...
Les pointillés signalent des appels asynchrones non bloquants : les réponses Google Ads ne conditionnent ni la réussite des handlers utilisateur, ni la latence perçue. L'étape 17 (nullification du gclid) est déclenchée après chaque upload ; elle est no-op tant que l'un des uploads applicables (deux ou trois `*_uploaded_at` selon que la conversion « First event sent » est active) est encore `NULL`. L'upload de la conversion « First event sent » (étapes 14-16) n'a lieu que lorsque cette fonctionnalité optionnelle est configurée.
