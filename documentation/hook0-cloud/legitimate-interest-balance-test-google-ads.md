...
- **Conversion « Signup »** : lorsque l'utilisateur valide son adresse email (handler `verify_email`), un champ `signup_uploaded_at` est positionné sur la ligne d'attribution et un appel `tokio::spawn` fire-and-forget déclenche un upload vers `uploadClickConversions` avec l'action de conversion Signup. La ligne d'attribution n'est pas supprimée ; elle est conservée pour les uploads ultérieurs (First event sent, First webhook delivered).
...
|--------|--------|-----------|
| Politique de confidentialité (sections « Conversion tracking » et 9b) | Réalisée, ré-alignement 1.5 à faire | `website/locales/{en,fr,de}/privacy-policy.js` (www.hook0.com/privacy-policy). La version publique décrit encore les finalités pré-1.5 (Signup + Activation) : son ré-alignement sur les trois finalités post-1.5 (Signup, First event sent, First webhook delivered) reste à faire au moment d'activer ces conversions en production, sous réserve d'une revue juridique / traduction native (cf. § 6). |
...
>
> **Note (révision 1.5) :** la présente révision 1.5 constitue un nouveau réexamen anticipé déclenché par le critère « changement de finalité » — **suppression** de la conversion « Activation » (création du premier jeton d'accès / première clé API). Son fait générateur est devenu un signal creux (proche de 100 %) depuis la création automatique d'une clé API par défaut à la configuration de l'organisation, et son rôle d'activation mi-tunnel est désormais porté par la conversion « First event sent » (usage réel du produit). Le tunnel passe de quatre à trois finalités (Signup → First event sent → First webhook delivered). La colonne `activation_uploaded_at` est supprimée du schéma et son verrou est retiré de la nullification du gclid : celle-ci est désormais conditionnée à Signup (toujours) plus First event sent / First webhook delivered lorsqu'elles sont actives. La borne maximale de rétention demeure **inchangée à 30 jours**. La balance tripartite a été re-évaluée : elle demeure favorable à l'intérêt légitime — le périmètre des données transmises se **réduit** (une finalité en moins, toujours le seul gclid), et la rétention n'est pas étendue. Le registre des traitements (art. 30) et la politique de rétention ont été mis à jour dans la même passe. Une action reste à réaliser : le ré-alignement de la politique de confidentialité **publique** (EN/FR/DE) sur les trois finalités post-1.5, sous réserve d'une revue juridique et d'une traduction native, au moment d'activer les conversions concernées en production.
...
flowchart LR
    ...
    User -->|9. ingère 1ᵉʳ événement| Ingest
    Ingest -->|10. INSERT event.event<br/>(gclid non touché)| DB
    Deliver -->|11. INSERT request_attempt<br/>succeeded_at (gclid non touché)| DB
    FirstEvJob -->|12. scan orgs gclid + événement<br/>si conversion active| DB
    FirstEvJob -.->|13. upload — First event conversion| GAdsAPI
    FirstEvJob -->|14. UPDATE first_event_uploaded_at| DB
    FirstWhJob -->|15. scan orgs gclid + webhook délivré<br/>si conversion active| DB
    FirstWhJob -.->|16. upload — First webhook delivered conversion| GAdsAPI
    FirstWhJob -->|17. UPDATE first_webhook_delivered_uploaded_at| DB
    DB -->|18. uploads applicables ok ?<br/>SET gclid = NULL| Null
