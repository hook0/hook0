...
| `HOOK0_API_URL` | `https://app.hook0.com` | The instance, without the `/api/v1` — the server adds it |
...
`HOOK0_API_URL` is the origin, not the API root: the tools carry `/api/v1/...` in their own paths and the server prepends it to anything that does not already start with it. Pointing this at `https://hook0.example.com/api/v1` therefore reaches `/api/v1/api/v1/...`.
...
Twenty-three, one per operation the API document tags `mcp`, named `<entity>.<operation>` after the operation id — the same names the SDKs group their methods under, so `applications.list` here is `ApplicationsApi.list` there.

The two tags are not the same set. The SDKs are generated from `public`, which is thirty-six operations; this server is generated from `mcp`, which is twenty-three. Twenty-one are in both. Fifteen reach the SDKs and not an assistant, and two — `organizations.get` and `organizations.list` — go the other way: they are tools here and no client has a group for them.
...
Which names are query parameters is stated on each tool rather than worked out from its schema, because a schema says what a caller fills in and not where any of it goes. `requestAttempts.list` is where it shows: seven of its arguments are filters travelling in the query string — `application_id`, `event.event_type_names`, `event_id`, `max_created_at`, `min_created_at`, `pagination_cursor`, `subscription_id` — and an argument left out is simply not asked for, which is what an optional filter has to mean.

A value the server cannot write as text — an object, an array, `null` — fills neither a path nor a query parameter. It reaches the API in the body or not at all.
...
That is the server's own restraint, not the API's. The token it holds still has every permission it was issued with, so a second process started without the variable has the whole set back. Where the restriction has to hold, [attenuate the service token](../../how-to-guides/manage-service-tokens.md#token-attenuation) as well: the API enforces that one.

## What it does not do

The eleven clients share four contracts — what they put on the wire, what they repeat, what they verify, what they bound. This server is held to the first and to none of the others, which is a statement about the code rather than an oversight to be fixed later.
...
Unlike the SDKs, nothing here reads an answer into a type. The server holds no model of what the API returns: a success travels on to the assistant as the JSON that arrived.
...
Three guided prompts — `create_webhook_subscription`, `debug_event_delivery` and `setup_application` — walk an assistant through the workflows those tools compose into.
