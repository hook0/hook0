...
```bash
# Forward webhooks to port 3000
hook0 listen 3000

# Forward to a specific path
hook0 listen http://localhost:8080/webhooks
...
# Forward example to your local server
...
```bash
# Send an event with inline payload
hook0 event send user.account.created \
  -l tenant=acme \
  -d '{"user_id": "usr_123", "email": "user@example.com"}'
