...
Here is what happens when you run `listen` then `event send`:
...
2. You create a subscription with that URL as the target. Then `hook0 event send` sends an event to the Hook0 API.
3. The API delivers the event to all matching subscriptions, including the relay URL.
4. The relay forwards the HTTP request through the WebSocket tunnel to your local machine.
