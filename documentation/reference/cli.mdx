---
title: CLI reference
description: Run Hook0 from your terminal. Send and replay events, manage subscriptions and event types, and receive webhooks on localhost through a built-in tunnel. Scriptable JSON output, shell completion.
---

# CLI reference

The Hook0 CLI is a command-line client for your [Hook0](https://hook0.com) account. Send events, manage subscriptions, receive webhooks locally, and automate your workflow. Everything you can do in the dashboard, from your terminal.
...
```mermaid
flowchart TD
    A["hook0 listen 3000"]:::external
    B["hook0 event send ..."]:::external
    C["Relay Server<br/>play.hook0.com"]:::hook0
    D["Hook0 API<br/>app.hook0.com/api/v1"]:::hook0
    E["localhost:3000<br/>(your server)"]:::customer

    A -- "1. connects via<br/>WebSocket" --> C
    B -- "3. sends event<br/>via HTTP" --> D
    D -- "4. API delivers to<br/>subscription URL<br/>(= relay URL from step 2)" --> C
    C -- "5. relay forwards<br/>via WebSocket" --> E
    ...
    click D "/openapi/intro" "Hook0 API"
    click C "/explanation/event-processing" "Event Processing"
```
