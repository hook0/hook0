...
/// The OpenTelemetry `Resource` describing this worker process. Shared verbatim by
/// the metrics, traces and logs pipelines so the three signals correlate on
/// identical `service.*`/`worker.name` attributes (including a stable
/// `service.instance.id`).
...
pub fn init(config: &Config, version: &str) -> Result<OtlpExporters, ExporterBuildError> {
    ...
    let resource = resource(config, version);
    ...
    if let Some(logs_endpoint) = &config.otlp_logs_endpoint {
        info!(
...
/// OTLP backend with the same correlation attributes — including the `trace_id`/`span_id`
/// already stamped on the delivery log lines.
pub fn init_logs(
    config: &Config,
    version: &str,
...
mod tests {
    ...
    fn all_three_signals_share_one_service_instance_id() {
        ...
        // different id, splitting one worker into three instances in Grafana.
