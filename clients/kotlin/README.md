...
**Send and verify webhooks, blocking or suspending, inside whichever runtime you brought**

<br/>

<img src="assets/kotlin-flow.svg" alt="How the Hook0 Kotlin SDK sits between your application and your users" width="850"/>
...
The Kotlin SDK for [Hook0](https://www.hook0.com/), the open source Webhooks-as-a-Service platform
...
Every call comes twice: one that blocks, and one that suspends. The suspending half is written
against `kotlin.coroutines`, the language's own, so it suspends inside whichever coroutine runtime
you brought: `kotlinx.coroutines`, Ktor's, Spring's, or none. **It brings nothing with it**, and
`kotlin-stdlib` is the only thing it declares.
...
- **The whole API, typed** - `data class`es for schemas, `enum class`es for closed lists, one exception per problem
- **Bounded everywhere** - `data class` options, so `copy` names the one bound you meant
- **Nothing read through `!!`** - a member the API does not declare required is a nullable type, everything else is not
...
That puts `com.hook0:hook0-client-kotlin:2.0.3` in `~/.m2`, where your own build resolves it:
...
Each problem the API can report is an exception of its own under a `sealed class ProblemException`,
so a `catch` may name one problem, or the base, or a `when` may match over the closed set and be
told by the compiler the day the API grows another.

### Headers, however they arrived

Verification takes them as a `Map<String, String>` or as a `List<Pair<String, String>>`, since a
request may carry the same header twice.

---

## Development

`clients/kotlin/src/main/kotlin/com/hook0/kotlin/generated/` is written by [`hook0-sdkgen`](https://gitlab.com/hook0/hook0/-/tree/master/clients/sdkgen)
...
Everything beside it, the transport, the retry loop, the signature verification, the JSON reader and
the suspending machinery, is hand-written and never regenerated, and so is `test/kotlin`, which sits
outside `src` on purpose, since a test file under a generated tree would be deleted without a word
...
The Hook0 Kotlin SDK is free and open source, released under the [MIT License](./LICENSE). Use it,
