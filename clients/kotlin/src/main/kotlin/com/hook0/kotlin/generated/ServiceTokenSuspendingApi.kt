 ...
 */
class ServiceTokenSuspendingApi(private val transport: Transport) {
   ...
   */
  suspend fun create(body: ServiceTokenPost): ServiceToken {
   ...
   */
  suspend fun delete(serviceTokenId: String, organizationId: String) {
   ...
   * @return what the API answered, once it has
   */
  suspend fun get(serviceTokenId: String, organizationId: String): ServiceToken {
    var path = "/api/v1/service_token/{service_token_id}"
    path = path.replace("{service_token_id}", Wire.pathSegment(serviceTokenId))
    val query = ArrayList<QueryParameter>()
    query.add(QueryParameter("organization_id", Wire.queryValue(organizationId)))
    return Problems.readAnswer(transport.requestSuspending("GET", path, query, null), ServiceToken.Companion::fromJson)
  }
   ...
   * @return what the API answered, once it has
   */
  suspend fun list(organizationId: String): List<ServiceToken> {
    val path = "/api/v1/service_token/"
    val query = ArrayList<QueryParameter>()
    query.add(QueryParameter("organization_id", Wire.queryValue(organizationId)))
    return Problems.readAnswer(
