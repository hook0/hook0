<template>
  <div>
        ...
        <hook0-card-content>
          ...
          <hook0-card-content-line>
            ...
            <template #content>
              <div class="flex flex-row">
                <hook0-select class="flex-none width-small" :options="httpTarget.METHODS"
                              v-model="subscription.target.method"></hook0-select>
                <hook0-input
                  type="text"
                  class="w-full ml-1"
          ...
          <hook0-card-content-line>
            ...
            <template #content>
              <hook0-input
                ...
                required
              >
                <template #helpText>Describe what your subscription will do
                </template>
              </hook0-input>
            </template>
          </hook0-card-content-line>
          ...
          <hook0-card-content-line>
            <template #label>
              <hook0-text>
                Select
                <hook0-button :to="{name: routes.EventTypesList}" target="_blank">event types</hook0-button>
                to listen to
    ...
    <SubscriptionsRemove v-if="!isNew" :subscription-id="subscription_id"
                         :subscription-name="subscription.name"></SubscriptionsRemove>
  </div>
...
function EventTypeNamesFromSelectedEventTypes(selectableEventTypes: SelectableEventType[]): string[] {
...
export default class SubscriptionsEdit extends Vue {
  ...
  _load() {
    ...
    if (this.subscription_id !== this.$route.params.subscription_id) {
      ...
      if (!this.isNew) {
        SubscriptionService.get(this.subscription_id).then((subscription: Subscription) => {
          ...
          this.httpTarget.headers = this.fromMap((subscription.target as unknown as Target).headers);
    ...
    EventTypeService.list(this.$route.params.application_id as string)
      .then(eventTypes => {
