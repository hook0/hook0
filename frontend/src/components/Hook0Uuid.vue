...
const display = computed(() =>
  props.truncated ? middleTruncate(props.value, 20) : props.value
);
</script>
...
<template>
  ...
  <span v-else class="hook0-uuid hook0-uuid__select-all" :class="{ 'hook0-uuid--linked': linked }">{{ display }}</span>
</template>
